...
function readAll(storeName) {
    return new Promise((resolve, reject) => {
        const request = serumDb.transaction(storeName, "readonly").objectStore(storeName).getAll();
        request.onsuccess = () => resolve(request.result || []);
        request.onerror = () => reject(request.error);
    });
}
...
function saveRecord(storeName, record) {
    return new Promise((resolve, reject) => {
        const request = serumDb.transaction(storeName, "readwrite").objectStore(storeName).put(record);
...
function deleteRecord(storeName, id) {
    return new Promise((resolve, reject) => {
        const request = serumDb.transaction(storeName, "readwrite").objectStore(storeName).delete(id);
...
function clearApplicationsForSerum(serumId) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            const records = (request.result || []).filter(item => item.serumId === serumId);
...
function serumDaysLabel(days) {
    return days.length === 7 ? "Daily" : `${days.length} day${days.length === 1 ? "" : "s"}`;
}

function normalizeSerum(formData) {
    return {
        id: editingSerumId || (crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`),
        name: formData.name,
...
function readSerumForm() {
    const days = [...document.querySelectorAll(".serumDay input:checked")].map(input => input.value);
    return {
...
function fillSerumForm(serum) {
    ...
    document.getElementById("serumFormTitle").textContent = serum ? "Update serum" : "Configure serum";
    document.getElementById("saveSerumBtn").innerHTML = serum ? '<i class="fa-solid fa-floppy-disk"></i> Update serum' : '<i class="fa-solid fa-plus"></i> Add serum';
    ...
    document.getElementById("serumActive").checked = serum ? serum.active !== false : true;
...
function scheduleForToday(serum) {
    return serum.active !== false && serum.days.includes(WEEKDAY_INDEX[new Date().getDay()]);
}

function appliedTodaySet() {
    const today = dayKey(new Date());
    return new Set(applications.filter(item => item.dayKey === today).map(item => item.serumId));
}
...
function renderSerumList() {
    ...
    if (!serums.length) {
        list.innerHTML = '<div class="serumEmpty">No serums configured yet. Add the name, dose, time, and schedule to start tracking.</div>';
        return;
    }
...
function renderChecklist() {
    ...
    document.getElementById("todayScheduledCount").textContent = todaySerums.length;
    document.getElementById("todayAppliedCount").textContent = todayApps.size;
    document.getElementById("todayPendingCount").textContent = Math.max(todaySerums.length - todayApps.size, 0);

    if (!todaySerums.length) {
        checklist.innerHTML = '<div class="serumEmpty">No serums are scheduled for today. Update your schedule in the configure panel.</div>';
        return;
    }

    checklist.innerHTML = todaySerums.map(serum => {
...
function drawChart(data, days) {
    ...
    data.forEach((item, index) => {
        ...
        if (days > 30) {
            ...
            const isBoundary = index === 0 || index === data.length - 1 || !previous || item.date.getMonth() !== previous.date.getMonth() || !next || item.date.getMonth() !== next.date.getMonth();
...
function updateStats() {
    ...
    const completionRate = totalScheduled ? Math.round((appliedTodaySet().size / totalScheduled) * 100) : 0;
...
function renderPeriodButtons() {
    ...
    container.innerHTML = PERIODS.map(period => `<button type="button" class="serumFilterButton ${period.value === activePeriod ? "active" : ""}" data-period="${period.value}">${period.label}</button>`).join("");
}
...
function serumValidation(form) {
    if (!form.name || !form.dosage || !form.time) return "Please add serum name, dosage, and time.";
...
document.addEventListener("DOMContentLoaded", async function() {
    try {
        ...
        document.getElementById("serumForm").addEventListener("submit", async function(event) {
        ...
        document.getElementById("serumList").addEventListener("click", async function(event) {
            ...
            if (editButton) {
                const serum = serums.find(item => item.id === editButton.dataset.id);
                if (serum) fillSerumForm(serum);
                window.scrollTo({ top: 0, behavior: "smooth" });
                return;
            }
            if (toggleButton) {
                const serum = serums.find(item => item.id === toggleButton.dataset.id);
                if (!serum) return;
            ...
            if (deleteButton) {
                const serum = serums.find(item => item.id === deleteButton.dataset.id);
        ...
        document.getElementById("dailyChecklist").addEventListener("change", async function(event) {
            const checkbox = event.target.closest(".markApplied");
            if (!checkbox) return;
            const serum = serums.find(item => item.id === checkbox.dataset.id);
            if (!serum) return;
            const today = dayKey(new Date());
            const existing = applications.find(item => item.serumId === serum.id && item.dayKey === today);
            if (checkbox.checked) {
                if (!existing) {
        ...
        document.getElementById("periodButtons").addEventListener("click", async function(event) {
